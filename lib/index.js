...
program
  ...
  .action(async (searchTerm, options) => {
    ...
    if (!searchTerm) {
      ...
      search = searchResponse.searchTerm;
      console.log(search);
    }
    ...
    const results = await startScraper(search, scraperOptions);
    ...
    const outputResponse = await prompt({
      type: 'select',
      name: 'outputType',
      message: 'What time of output?',
      choices: ['fzf', 'shellfile', 'aria2']
    });
